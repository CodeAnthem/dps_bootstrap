...
# ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# Date:          Created: 2025-10-12 | Modified: 2025-10-17
...
cloneRepo() {
    if git clone --quiet "$REPO_URL" "$REPO_PATH" 2>/dev/null; then
        printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ✅ Successfully cloned repository" >&2
    else
        printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ❌ Failed to clone repository" >&2
...
resetRepo() {
    ...
    then
        printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ✅ Successfully reset repository" >&2;
    else
        printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ❌ Failed to reset repository" >&2;
        exit 1
    fi
}
...
checkUntrackedFiles() {
    ...
    if [[ -n "$untracked" ]]; then
        printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ⚠️ Untracked files detected (potential security risk)" >&2
        ...
        case "${answer^^}" in
            Y)
                ...
                printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ✅ Untracked files removed" >&2
                ;;
            *)
                printf " %(%Y-%m-%d %H:%M:%S)T %s\n" -1 "Quickstart | ⚠️ Proceeding with untracked files present" >&2
                ;;
        esac
    fi
}
