...
nds_bundle_contrib_core() {
    ...
    if declare -f nds_cfg_export_script &>/dev/null; then
        nds_bundle_register_text "nds-config.env" "$(nds_cfg_export_script)"
    fi
    ...
    [[ -f "$install_log" ]] && nds_bundle_register_file "logs/install.log" "$install_log"
    [[ -f "$diag_log" ]] && nds_bundle_register_file "logs/diag.log" "$diag_log"
    [[ -f "$session_log" ]] && nds_bundle_register_file "logs/session.log" "$session_log"
