...
nds_gh_prefetch() {
    ...
    declare -f nds_install_log &>/dev/null && nds_install_log "gh: downloading CLI via nixpkgs#gh" || true
    ...
    if declare -f nds_step_start &>/dev/null; then
        ...
        build_out=$(_nds_gh_nix build --no-link --print-out-paths nixpkgs#gh 2>&1) || rc=$?
        {
            printf '\n=== %s ===\n' "$label"
            printf '%s\n' "$build_out"
        } >>"$logfile"
