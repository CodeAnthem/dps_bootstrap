...
RAW_BASE="${NDS_BOOTSTRAP_RAW_BASE:-https://raw.githubusercontent.com/CodeAnthem/dps_bootstrap/main/src/runtime-tools}"
...
_switch_self_update() {
    ...
    _switch_info "updated. Put ${dest} first on PATH (extraInit already prefers it after rebuild)."
    _switch_info "run now: ${dest}/nds-switch"
    exit 0
}
...
[[ -d "$FLAKE_ROOT" ]] || _switch_die "flake root missing: ${FLAKE_ROOT}"
[[ -d "${FLAKE_ROOT}/.git" ]] || _switch_die "not a git checkout: ${FLAKE_ROOT}"
...
if git rev-parse --is-shallow-repository 2>/dev/null | grep -qx true; then
    _switch_info "unshallowing clone for updates"
...
if [[ "${ahead:-0}" -gt 0 ]]; then
    _switch_die "local branch is ahead of ${REMOTE_REF} by ${ahead} commit(s).
...
if [[ "${behind:-0}" -eq 0 ]]; then
    _switch_info "already up to date with ${REMOTE_REF}"
else
    _switch_info "fast-forward ${behind} commit(s) from ${REMOTE_REF}"
    ...
    if [[ -n "$host_dir" ]]; then
        for f in facter.json hardware-configuration.nix nds-boot.nix machine.nix; do
            ...
            if git check-ignore -q "${host_dir}/${f}" 2>/dev/null \
                ...
                _switch_info "parked ${host_dir}/${f} -> ${stash_dir}"
            fi
        done
    fi
    ...
    if ! git pull --ff-only origin "$branch"; then
        [[ -n "$stash_dir" ]] && _switch_info "host facts parked at ${stash_dir}"
        _switch_die "fast-forward failed — resolve manually in ${FLAKE_ROOT}"
    fi
    ...
    if [[ -n "$stash_dir" && -d "$stash_dir" ]]; then
        while IFS= read -r -d '' parked; do
            ...
            if [[ -e "${FLAKE_ROOT}/${rel}" ]]; then
                _switch_info "keeping remote ${rel}; parked copy at ${parked}"
            else
                mkdir -p "$(dirname "${FLAKE_ROOT}/${rel}")"
                mv "$parked" "${FLAKE_ROOT}/${rel}"
                _switch_info "restored ${rel}"
            fi
...
_switch_info "nixos-rebuild switch --flake ${FLAKE_ROOT}#${HOST_NAME}"
