...
nds_settings_manager_load() {
    ...
    nds_import_file "${sm_dir}/state/aa-bridge.sh" || return 1
    nds_import_file "${sm_dir}/state/scoped-arrays.sh" || return 1
    nds_import_file "${SCRIPT_DIR}/settings/system-vars/load.sh" || return 1
    nds_system_vars_load "${SCRIPT_DIR}/settings/system-vars" || return 1
