    ...
    while true; do
        ...
        if [[ "$value" =~ ^[0-9]+$ ]]; then
            if [[ "$value" -ge 0 && "$value" -le 32 ]]; then
                ...
                local dotted
                dotted=$(_netmask_cidr_to_dotted "$value")
                echo "$dotted"
                return 0
            else
        ...
        if _netmask_validate "$value"; then
            echo "$value"
            return 0
        else
...
_netmask_errorCode() {
    echo "Invalid network mask (use dotted decimal, e.g., 255.255.255.0)"
}

# Auto-register this settingType
nds_cfg_settingType_register "netmask"
