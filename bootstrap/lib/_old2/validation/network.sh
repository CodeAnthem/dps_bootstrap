...
# Description:   Validation Library - Network
# Feature:       Network-related validation functions (IP, hostname, netmask, subnet, port)
# ==================================================================================================
...
cidr_to_netmask() {
    ...
    for ((i=0; i<4; i++)); do
        local bits_in_octet=$((cidr > 8 ? 8 : cidr))
        [[ $bits_in_octet -lt 0 ]] && bits_in_octet=0
        
        local octet_value=0
        for ((bit=0; bit<bits_in_octet; bit++)); do
            octet_value=$((octet_value + (1 << (7 - bit))))
        done
