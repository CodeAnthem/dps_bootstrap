...
# Register flake experimental features and base nix settings (classic install).
# Usage: nds_nixcfg_base_auto
nds_nixcfg_base_auto() {
    nds_nixcfg_register "nix" 'nix.settings.experimental-features = [ "nix-command" "flakes" ];' 5
}

# Build a full classic configuration.nix from configurator answers.
# Usage: nds_nixcfg_build_classic_auto
