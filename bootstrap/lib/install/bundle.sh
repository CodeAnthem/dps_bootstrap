...
_nds_install_bundle_quickstart() {
    local dest="$1"
    local hostname admin_user ssh_port ssh_pw_auth admin_ssh_key port_opt
...
- **SSH from your PC:**
  ...
  \`\`\`bash
  ssh ${admin_user}@<machine-ip>${port_opt}
  \`\`\`
...
- **SSH from your PC** (key-only — password login is off, use your key):
  ...
  \`\`\`bash
  ssh -i <your-private-key> ${admin_user}@<machine-ip>${port_opt}
  \`\`\`
EOF
        fi
...
EOF
        ...
        if [[ "$encryption" == "true" && "$remote_unlock" == "true" ]]; then
            unlock_ip="<machine-ip>"
            ip_note="With DHCP the initrd gets its **own** lease, so its IP is usually **not** the machine's normal IP — check your router/DHCP logs for the address shown while the LUKS prompt is up."
