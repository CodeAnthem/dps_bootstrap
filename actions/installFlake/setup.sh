...
action_preview() {
    ...
    nds_ui_b ""
    nds_ui_b "For a private repo on the live ISO, NDS verifies the full flake closure"
    nds_ui_b "before partitioning. Use an HTTPS token (memory-only) or SSH deploy key."
    nds_ui_b "Remote install (nixos-anywhere) requires SSH keys on the operator machine."
    nds_ui_b ""
